...
import { CreditCard, Smartphone, Wallet, ArrowDownToLine, Loader2, CheckCircle2, Clock, XCircle, Terminal, Code2 } from "lucide-react";
...
const { status, amount, netAmount } = await res.json();
            ...
            <CardContent>
              {txs.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-xs mt-1">Use the API below to start accepting payments.</p>
                </div>
              ) : (
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-muted-foreground">Check payment status. Poll this after the customer returns from the redirect URL.</p>
              <CodeBlock curl={statusCurl} node={statusNode} />

              <div className="bg-gray-50 rounded-lg p-3 text-sm">
                <p className="font-semibold mb-1">Status values</p>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  {[["pending", "amber", "Awaiting payment"], ["completed", "green", "Payment successful — balance credited"], ["failed", "red", "Payment failed"], ["cancelled", "gray", "Customer cancelled"]].map(([s, c, d]) => (
                    <div key={s} className="flex items-center gap-2">
                      <Badge variant="outline" className={`text-xs border-${c}-400 text-${c}-700`}>{s}</Badge>
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 space-y-2">
              ...
              <div className="text-sm text-blue-700 space-y-1">
                ...
                <p>4. Request a withdrawal from this page — admin processes it to your M-Pesa</p>
              </div>
            </div>
